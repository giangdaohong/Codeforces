    ...
    static class TaskD {
        ...
        public void solve(int testNumber, QuickScanner in, QuickWriter out) {
            ...
            for (int remQ = in.nextInt(); remQ > 0; --remQ) {
                ...
                int x2 = in.nextInt();
                int y2 = in.nextInt();
                int left = 1, right = Math.min(x2 - x1, y2 - y1), res = 0;
                while (left <= right) {
                    int medium = (left + right) >> 1;
                    int startX = x1 + medium - 1;
                    int startY = y1 + medium - 1;
                    int calMax = rmq.calc(startX, startY, x2, y2);
                    if (calMax >= medium) {
                        res = medium;
                        left = medium + 1;
                    } else {
        ...
        class IntRangeMaximumQuery2D {
            ...
            public void init(int n, int m, int[][] value) {
                for (int bitI = 0; 1 << bitI <= n; ++bitI) {
                    for (int bitJ = 0; 1 << bitJ <= m; ++bitJ) {
                        for (int i = 0; i <= n - (1 << bitI); ++i)
                            for (int j = 0; j <= m - (1 << bitJ); ++j) {
                                if (bitI > 0) {
                                    rmq[bitI][bitJ][i][j] = Math.max(rmq[bitI - 1][bitJ][i][j], rmq[bitI - 1][bitJ][i + (1 << (bitI - 1))][j]);
                                } else if (bitJ > 0) {
                                    rmq[bitI][bitJ][i][j] = Math.max(rmq[bitI][bitJ - 1][i][j], rmq[bitI][bitJ - 1][i][j + (1 << (bitJ - 1))]);
                                } else {
            ...
            public int calc(int x1, int y1, int x2, int y2) {
                ...
                return Math.max(Math.max(Math.max(rmq[bitX][bitY][x1][y1], rmq[bitX][bitY][x2 - (1 << bitX)][y1]), rmq[bitX][bitY][x1][y2 - (1 << bitY)]), rmq[bitX][bitY][x2 - (1 << bitX)][y2 - (1 << bitY)]);
            }
